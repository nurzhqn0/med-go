...
func main() {
	...
	doctorService, err := doctorapp.New(startupCtx, config.DoctorAddress, config.DatabaseURL, config.NATSURL, "../../doctor-service/migrations", config.RedisURL, config.CacheTTL, config.RateLimitRPM)
	if err != nil {
